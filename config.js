...
{
    return context.workspaceState.get( 'expanded', vscode.workspace.getConfiguration( 'tagexplorer.tree' ).get( 'expanded', false ) );
}

function shouldFlatten()
{
    return context.workspaceState.get( 'flat', vscode.workspace.getConfiguration( 'tagexplorer.tree' ).get( 'flat', false ) );
}

function shouldShowTagsOnly()
{
    return context.workspaceState.get( 'tagsOnly', vscode.workspace.getConfiguration( 'tagexplorer.tree' ).get( 'tagsOnly', false ) );
}

function shouldShowCounts()
{
    return vscode.workspace.getConfiguration( 'tagexplorer.tree' ).get( 'showCountsInTree', false );
}

function showFilterCaseSensitive()
{
    return vscode.workspace.getConfiguration( 'tagexplorer.tree' ).get( 'filterCaseSensitive', false );
}

function isRegexCaseSensitive()
{
    return vscode.workspace.getConfiguration( 'tagexplorer.regex' ).get( 'regexCaseSensitive', true );
}

function showBadges()
{
    return vscode.workspace.getConfiguration( 'tagexplorer.tree' ).get( 'showBadges', false );
}
...
function regex()
{
    return {
        // tags: tags(),
        regex: vscode.workspace.getConfiguration( 'tagexplorer.regex' ).get( 'regex' ),
        caseSensitive: vscode.workspace.getConfiguration( 'tagexplorer.regex' ).get( 'regexCaseSensitive' )
    };
}
...
function ripgrepPath()
{
    ...
    rgPath = exePathIsDefined( vscode.workspace.getConfiguration( 'tagexplorer.ripgrep' ).ripgrep );
...
function tags()
{
    return vscode.workspace.getConfiguration( 'tagexplorer.general' ).tags.sort().reverse();
}

function shouldSortTagsOnlyViewAlphabetically()
{
    return vscode.workspace.getConfiguration( 'tagexplorer.tree' ).sortTagsOnlyViewAlphabetically;
}

function labelFormat()
{
    return vscode.workspace.getConfiguration( 'tagexplorer.tree' ).labelFormat;
}

function tooltipFormat()
{
    return vscode.workspace.getConfiguration( 'tagexplorer.tree' ).tooltipFormat;
}

function shouldShowHighlights( scheme )
{
    var schemes = vscode.workspace.getConfiguration( 'tagexplorer.highlights' ).schemes;
    return schemes && schemes.length && schemes.indexOf( scheme ) !== -1;
}

function shouldUseBuiltInFileExcludes()
{
    var useBuiltInExcludes = vscode.workspace.getConfiguration( 'tagexplorer.filtering' ).useBuiltInExcludes;
...
{
    return vscode.workspace.getConfiguration( 'tagexplorer.filtering' ).ignoreGitSubmodules;
}

function shouldCompactFolders()
{
    return vscode.workspace.getConfiguration( 'explorer' ).compactFolders &&
        vscode.workspace.getConfiguration( 'tagexplorer.tree' ).disableCompactFolders !== true;
}
